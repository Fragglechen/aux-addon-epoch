...
function M.new(parent, on_click, selected)
	...
	scroll_frame:SetScript('OnVerticalScroll', function(self, offset)
		FauxScrollFrame_OnVerticalScroll(self, offset, ROW_HEIGHT, function() render(this.item_listing) end)
	end)
...
function M.scroll_to(item_listing, target_record)
	if not target_record or not item_listing.item_records then return end
	local index = nil
	for i = 1, getn(item_listing.item_records) do
		if item_listing.item_records[i] == target_record then
			index = i
			break
		end
	end
	if not index then return end
	local visible = getn(item_listing.rows)
	local total = getn(item_listing.item_records)
	local current_offset = FauxScrollFrame_GetOffset(item_listing.scroll_frame)
	if index - 1 >= current_offset and index - 1 < current_offset + visible then
		return  -- already visible, don't disturb manual scroll position
	end
	local new_offset = max(0, min(index - 1, total - visible))
	item_listing.scroll_frame:SetVerticalScroll(new_offset * ROW_HEIGHT)
	render(item_listing)
end
